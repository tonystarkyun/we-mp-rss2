  ...
  <div class="add-patent">
    ...
    <a-card>
      <a-form
        ...
        @submit="handleSubmit"
      >
        <a-form-item label="专利网站链接" field="url">
          <a-input
            v-model="form.url"
            placeholder="请输入专利网站URL，例如：https://patents.google.com"
            allow-clear
          >
...
import { addPatent, testPatentCrawl } from '@/api/patent'
...
const rules = {
  url: [
    { required: true, message: '请输入专利网站URL' },
    {
...
const testCrawl = async () => {
  ...
  try {
    const response = await testPatentCrawl(form.value.url, 5)
    ...
    if (response.code === 0 || response.data?.code === 0 || response.crawl_success === true) {
      ...
      Message.success('爬取测试成功！已自动填充专利网站信息')
    } else {
      testResult.value = {
        success: false,
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const response = await addPatent({
      url: form.value.url,
      name: form.value.name,
      avatar: '', // 使用默认头像
      description: form.value.description
    ...
    console.log('添加专利API响应:', response)
    
    if (response.code === 0 || response.data?.code === 0 || response.message === '专利链接添加成功') {
      Message.success('专利链接添加成功！')
      router.push('/patents')
    } else {
      Message.error(response.message || response.data?.message || '添加专利链接失败')
    }
  } catch (error) {
    console.error('添加专利错误:', error)
    Message.error(error.message || '添加专利链接失败')
  } finally {
    loading.value = false
  }
}
