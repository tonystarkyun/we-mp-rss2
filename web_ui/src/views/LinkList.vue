<template>
  ...
  <a-layout class="link-list">
      ...
      <a-card :bordered="false" title="文献查询" 
        ...
        <template #extra>
           <a-dropdown>
              ...
              <template #content>
                <a-doption @click="showAddModal"><template #icon><icon-plus /></template>添加文献</a-doption>
                <!-- <a-doption @click="exportLinks"><template #icon><icon-export /></template>导出文献</a-doption>
                <a-doption @click="importLinks"><template #icon><icon-import /></template>导入文献</a-doption>
        ...
        <div style="display: flex; flex-direction: column;; background: #fff">
            <a-list :data="linkList" :loading="linkLoading" bordered>
              <template #item="{ item, index }">
                <a-list-item @click="handleLinkClick(item.id)" :class="{ 'active-link': activeLinkId === item.id }"
                 ...
                 <div style="display: flex; align-items: center;">
                   ...
                   <a-button v-if="activeLinkId === item.id && item.id!=''" size="mini" type="text" status="danger" @click="$event.stopPropagation(); deleteLink(item.id)" >
            ...
            <a-pagination :total="linkPagination.total" simple  @change="handleLinkPageChange" :show-total="true" style="margin-top: 1rem;"/>
        </div>
      </a-card>
    </a-layout-sider>
    ...
    <a-layout-content :style="{ padding: '20px', width: '100%'}" >
      ...
      :subtitle="'管理您的文献查询内容'" :show-back="false">
          <template #extra>
          <a-space>
      ...
      <a-card style="border:0">
        <a-alert type="success" closable>{{activeFeed?.mp_intro||"请选择一个文献源进行管理,搜索文章后再点击订阅会有惊喜哟！！！"}}</a-alert>
...
const handleLinkPageChange = (page: number, pageSize: number) => {
  linkPagination.value.current = page
...
const handleLinkClick = (linkId: string) => {
  ...
  activeFeed.value=linkList.value.find(item => item.id === activeLinkId.value)
...
const exportLinks = async () => {
  try {
    const response = await exportLinksApi();
...
const importLinks = async () => {
  try {
    ...
    input.onchange = async (e) => {
      ...
      const response = await importLinksApi(file);
...
const openRssFeed = () => {
  ...
  if (!activeLinkId.value) {
    window.open(`/feed${search}/all.${format}`, '_blank')
    return
  }
  const activeLink = linkList.value.find(item => item.id === activeLinkId.value)
  if (activeMp) {
    window.open(`/feed${search}/${activeLinkId.value}.${format}`, '_blank')
  }
}
...
const handleRefresh = () => {
  ...
  updateLinks(activeLinkId.value, {
...
const fetchLinkList = async () => {
  ...
  try {
    const res = await getLinks({
      page: linkPagination.value.current - 1,
      pageSize: linkPagination.value.pageSize
    })
    
    // 调试响应结构
    console.log('链接API响应:', res)
    ...
    linkList.value = listData.map(item => ({
      id: item.id || item.url,
    ...
    // 添加'全部'选项
    linkList.value.unshift({
      id: '',
      name: '全部',
      avatar: '/static/logo.svg',
      description: '显示所有文献文章',
      article_count: totalCount
    });
    linkPagination.value.total = totalCount
  } catch (error) {
...
const deleteLink = async (linkId: string) => {
  try {
    Modal.confirm({
      title: '确认删除',
      content: '确定要删除该文献链接吗？删除后将无法恢复。',
      okText: '确认',
      cancelText: '取消',
      onOk: async () => {
        await deleteLinkApi(linkId);
        Message.success('文献链接删除成功');
        fetchLinkList();
      },
      onCancel: () => {
        Message.info('已取消删除操作');
    ...
    console.error('删除文献链接失败:', error);
    Message.error('删除文献链接失败，请稍后重试');
  }
}
...
const exportArticles = () => {
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `articles_${activeLinkId.value || 'all'}_${new Date().toISOString().slice(0, 10)}.json`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
