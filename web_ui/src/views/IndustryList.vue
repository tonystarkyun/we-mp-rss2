<template>
  ...
  <a-layout class="industry-list">
      ...
      <a-card :bordered="false" title="行业动态" 
        ...
        <template #extra>
           <a-dropdown>
              ...
              <template #content>
                <a-doption @click="showAddModal"><template #icon><icon-plus /></template>添加行业</a-doption>
                <!-- <a-doption @click="exportIndustries"><template #icon><icon-export /></template>导出行业</a-doption>
                <a-doption @click="importIndustries"><template #icon><icon-import /></template>导入行业</a-doption>
        ...
        <div style="display: flex; flex-direction: column;; background: #fff">
            <a-list :data="industryList" :loading="industryLoading" bordered>
              <template #item="{ item, index }">
                <a-list-item @click="handleIndustryClick(item.id)" :class="{ 'active-industry': activeIndustryId === item.id }"
                 ...
                 <div style="display: flex; align-items: center;">
                   ...
                   <a-button v-if="activeIndustryId === item.id && item.id!=''" size="mini" type="text" status="danger" @click="$event.stopPropagation(); deleteIndustry(item.id)" >
            ...
            <a-pagination :total="industryPagination.total" simple  @change="handleIndustryPageChange" :show-total="true" style="margin-top: 1rem;"/>
        </div>
      </a-card>
    </a-layout-sider>
    ...
    <a-layout-content :style="{ padding: '20px', width: '100%'}" >
      ...
      :subtitle="'管理您的行业动态内容'" :show-back="false">
          <template #extra>
          <a-space>
      ...
      <a-card style="border:0">
        <a-alert type="success" closable>{{activeFeed?.mp_intro||"请选择一个行业进行管理,搜索文章后再点击订阅会有惊喜哟！！！"}}</a-alert>
...
import { getIndustries, updateIndustries, deleteIndustryApi, exportIndustries as exportIndustriesApi, importIndustries as importIndustriesApi } from '@/api/industry'
...
const handleIndustryPageChange = (page: number, pageSize: number) => {
  industryPagination.value.current = page
  industryPagination.value.pageSize = pageSize
  fetchIndustryList()
}
...
const handleIndustryClick = (industryId: string) => {
  ...
  activeFeed.value=industryList.value.find(item => item.id === activeIndustryId.value)
...
const fetchArticles = async () => {
  ...
  try {
    console.log('请求参数:', {
      ...
      industry_id: activeIndustryId.value
    })
    ...
    articles.value = (res.list || []).map(item => ({
      ...item,
      mp_name: item.mp_name || item.account_name || '行业文章',
...
const exportIndustries = async () => {
  try {
    const response = await exportIndustriesApi();
    ...
    Message.error(error?.message || '导出行业失败');
  }
};

const importIndustries = async () => {
  try {
    ...
    input.onchange = async (e) => {
      ...
      const response = await importIndustriesApi(file);
      Message.info(response?.message || "导入成功");
    };
    input.click();
  } catch (error) {
    Message.error(error?.message || '导入行业失败');
  }
};
...
const openRssFeed = () => {
  ...
  const activeIndustry = industryList.value.find(item => item.id === activeIndustryId.value)
  if (activeIndustry) {
    window.open(`/feed${search}/${activeIndustryId.value}.${format}`, '_blank')
  }
}
...
const handleRefresh = () => {
  ...
  updateIndustries(activeIndustryId.value, {
...
onMounted(() => {
  ...
  fetchIndustryList().then(() => {
    console.log('行业列表获取完成')
    fetchArticles()
  }).catch(err => {
...
const fetchIndustryList = async () => {
  ...
  try {
    const res = await getIndustries({
      page: industryPagination.value.current - 1,
      pageSize: industryPagination.value.pageSize
    })
    
    // 调试响应结构
    console.log('行业API响应:', res)
    ...
    industryList.value = listData.map(item => ({
      id: item.id || item.url,
    ...
    // 添加'全部'选项
    industryList.value.unshift({
      id: '',
      name: '全部',
      avatar: '/static/logo.svg',
      description: '显示所有行业文章',
      article_count: totalCount
    });
    industryPagination.value.total = totalCount
  } catch (error) {
...
const deleteIndustry = async (industryId: string) => {
  try {
    Modal.confirm({
      title: '确认删除',
      content: '确定要删除该行业链接吗？删除后将无法恢复。',
      okText: '确认',
      cancelText: '取消',
      onOk: async () => {
        await deleteIndustryApi(industryId);
        Message.success('行业链接删除成功');
        fetchIndustryList();
      },
      onCancel: () => {
        Message.info('已取消删除操作');
    ...
    console.error('删除行业链接失败:', error);
    Message.error('删除行业链接失败，请稍后重试');
  }
}
...
const exportArticles = () => {
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `articles_${activeIndustryId.value || 'all'}_${new Date().toISOString().slice(0, 10)}.json`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
