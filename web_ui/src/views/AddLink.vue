  ...
  <div class="add-link">
    ...
    <a-card>
      <a-form
        ...
        @submit="handleSubmit"
      >
        <a-form-item label="文献网站链接" field="url">
          <a-input
            v-model="form.url"
            placeholder="请输入文献网站URL，例如：https://arxiv.org"
            allow-clear
          >
...
import { addLink, testWebsiteCrawl } from '@/api/link'
...
const rules = {
  url: [
    { required: true, message: '请输入文献网站URL' },
    {
...
const testCrawl = async () => {
  ...
  try {
    const response = await testWebsiteCrawl(form.value.url, 5)
    ...
    if (response.code === 0 || response.data?.code === 0 || response.crawl_success === true) {
      ...
      Message.success('爬取测试成功！已自动填充文献网站信息')
    } else {
      testResult.value = {
        success: false,
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // Arco Design的validate方法在成功时返回undefined，失败时抛出异常或返回错误信息
    // 我们直接进行手动验证
    if (!form.value.url) {
      Message.error('请输入文献网站URL')
      return
    }
    ...
    console.log('添加链接API响应:', response)
    
    if (response.code === 0 || response.data?.code === 0 || response.message === '链接添加成功') {
      Message.success('文献链接添加成功！')
      router.push('/links')
    } else {
      Message.error(response.message || response.data?.message || '添加文献链接失败')
    }
  } catch (error) {
    console.error('添加链接错误:', error)
    Message.error(error.message || '添加文献链接失败')
  } finally {
    loading.value = false
  }
}
